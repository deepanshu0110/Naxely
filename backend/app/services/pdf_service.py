...
PDF_FOOTER_URL = 'https://naxely.com/?utm_source=pdf_footer&utm_medium=referral&utm_campaign=client_report'
...
def _link_footer_span(canvas, font_name, font_size, prefix_text, link_text):
    """Add an invisible URI annotation exactly over one span of the footer line.

    Text itself is already painted by the caller via drawString at (MARGIN, 30);
    this only adds the clickable rect. (x1, y1, x2, y2) covers the link span's
    baseline ± a small padding so it matches the visible glyphs.
    """
    x1 = MARGIN + canvas.stringWidth(prefix_text, font_name, font_size)
    x2 = x1 + canvas.stringWidth(link_text, font_name, font_size)
    canvas.linkURL(PDF_FOOTER_URL, (x1, 30 - 2, x2, 30 + font_size), relative=0)
...
def _on_page(canvas, doc, add_watermark=False, is_white_label=False, is_free=False, company_name=''):
    ...
    if is_white_label:
        ...
        _link_footer_span(canvas, 'IBMPlexSans', 9, 'Made with Naxely — ', 'naxely.com')
    else:
        canvas.setFont('IBMPlexSans', 8)
        canvas.setFillColor(HexColor(MUTED))
        canvas.drawString(MARGIN, 30, f'Naxely Report — {datetime.now().strftime("%B %d, %Y")}')
        _link_footer_span(canvas, 'IBMPlexSans', 8, '', 'Naxely Report')
