...
WATERMARK_TEXT = 'Generated by Databrief'

OUTPUT_ROOT = Path('/tmp/databrief')
...
def _download_logo(logo_url: str, brand_color_hex: str) -> str | None:
    try:
        req = urllib.request.Request(logo_url, headers={'User-Agent': 'Databrief/1.0'})
...
def _on_page(canvas, doc, add_watermark=False, is_white_label=False, company_name=''):
    ...
    if is_white_label:
        ...
        canvas.drawString(MARGIN, 30, f'Databrief Report — {datetime.now().strftime("%B %d, %Y")}')
...
def build_sync(
    ...
    if logo_url:
        logo_path = _download_logo(logo_url, brand_color)

    if logo_path:
        img = PILImage.open(logo_path)
        w_px, h_px = img.size
        draw_w = min(w_px, content_width)
        draw_h = draw_w * (h_px / w_px)
        if draw_h > 120:
            draw_w = draw_w * (120 / draw_h)
            draw_h = 120
        story.append(Spacer(1, inch * 1.5))
        story.append(Image(logo_path, width=draw_w, height=draw_h))
    ...
    if date_from and date_to and date_from != 'N/A':
        story.append(Paragraph(f'From {date_from} to {date_to}', cover_sub_style))
